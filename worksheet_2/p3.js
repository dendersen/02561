...
//gully controlled by click logic
var tempArray = [];
var tempColorArray = [];
...
function draw(positions, colors, seconds){
  ...
  if (VertexArray.length != VertexColorArray.length){
    console.log("VertexArray and VertexColorArray are not the same length!");
  }
  for (let i = 0; i < VertexArray.length && i < VertexColorArray.length; i++){
    add_point(positions, colors, VertexArray[i], 0.1, VertexColorArray[i]);
  }
}
